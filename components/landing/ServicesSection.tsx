...
const services = [
  {
    ...
    desc: "Export of high-quality Taiwanese sedan cars, SUVs, heavy-duty logistics trucks, refrigerated vans, and B2B transit buses.",
    icon: Truck,
  },
  {
...
export default function ServicesSection() {
  return (
    <section id="services" className="py-24 md:py-32 bg-brand-black text-white">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        
        {/* Header */}
        <div className="max-w-2xl mb-20">
          <Reveal y={20}>
            <span className="text-xs font-semibold text-brand-red uppercase tracking-widest block mb-3">Capabilities & Infrastructure</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((svc, idx) => (
            <Reveal key={svc.title} y={24} delay={idx * 0.05}>
              <div className="h-full bg-brand-carbon p-8 rounded-2xl border border-white/5 hover:border-brand-red/30 transition-all duration-300 group flex flex-col justify-between">
                <div>
                  {/* Icon */}
                  <div className="w-12 h-12 rounded-xl bg-brand-black text-brand-red flex items-center justify-center mb-6 group-hover:scale-105 transition-all">
                    <svc.icon className="w-6 h-6" />
                  </div>

                  {/* Text details */}
                  <h3 className="font-display font-black text-lg text-white mb-3 group-hover:text-brand-red transition-colors">
                    {svc.title}
                  </h3>
                ...
                <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between text-xs font-bold text-white/50 group-hover:text-white transition-colors">
                  ...
                  <ArrowRight className="w-4 h-4 text-brand-red opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                </div>
              </div>
            </Reveal>
          ))}
